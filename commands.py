...
def edit_note_function():
    ...
    if note:
        ...
        new_tags = [tag.strip() for tag in input(f"Enter new tags (current: {', '.join(note.tags)}): ").split(",") if tag.strip()]
...
def search_notes_function():
    tag_completer = WordCompleter(notes_manager.get_autocomplete_words(), ignore_case=True)
